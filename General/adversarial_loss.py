...
from monai.networks.nets import PatchDiscriminator as Discriminator
...
class PatchDiscriminator(nn.Module):
    def __init__(self, 
        ...
        super().__init__()
        self.block = Discriminator(spatial_dims=2, 
                                   num_layers_d=3, 
    ...
    def forward(self, x):
        ...
        if x.dim() == 4: # (B, C, H, W)
            output = self.block(x)
        elif x.dim() == 5: # (B, C, H, W, D)           
            B, C, H, W, D = x.size()
            output = self.block(x.permute((0, -1, 1, 2, 3)).view(B*D, C, H, W))
