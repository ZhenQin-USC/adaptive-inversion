...
class Regularization:
    ...
    def __call__(self, zm):
        ...
        elif self.mode == 'TV':  # Total Variation regularization
            reg = self.total_variation(zm)
        else:
...
class GradientBasedSpatioTemporalProxy:
    def __init__(self, 
                 ...
                 update_scales:List=None,
                 scale_index_map:Union[torch.Tensor]=None,
                 z0:np.ndarray=None
                 ):
        ...
        # Parameters used for adaptive update
        self.update_scales = update_scales                                     # A list contains the scales to update/focus
        if scale_index_map is not None:
            self.update_maps = scale_index_map.unsqueeze(0).unsqueeze(0).to(device) # (1, 1, H, W, D)
        else:
            self.update_maps = scale_index_map
        if z0 is not None:
            self.z0 = torch.tensor(z0, dtype=torch.float32, requires_grad=False).to(device)
        else:
            self.z0 = z0
    ...
    def _masker(self, zm: torch.Tensor) -> torch.Tensor:
        """
        zm: (1, C, B, H, W) - Input tensor to be updated
        z0: (1, C, B, H, W) - Reference tensor to fill non-updated regions
        update_scales: List of scale values indicating which regions should be updated
        update_map: (1, 1, B, H, W) - A tensor containing scale indices for each element
        """

        z0 = self.z0
        update_scales = self.update_scales
        update_map = self.update_maps

        if z0 is None:
            if update_scales is not None:
                raise ValueError("z0 is required when update_scales is provided.")
            return zm  # If z0 is not provided and update_scales is also None, return zm unchanged

        if update_scales is not None and update_map is not None:

            # Initialize a mask with False values
            mask = torch.zeros_like(update_map, dtype=torch.bool)  

            # Mark positions where update_map matches any value in update_scales
            for scale in update_scales:
                mask |= (update_map == scale)  

            # Expand mask dimensions to match zm and z0
            mask = mask.expand_as(zm) 

            # Use torch.where to retain zm values at masked positions, otherwise use self.z0
            zm = torch.where(mask, zm, z0)   # mask = 0 (False) --> return z0 (no update)
...
class GradientBasedProxyOptimizer(GradientBasedSpatioTemporalProxy):
    ...
    """
    def __init__(self, 
                 predictor, autoencoder, measurement, selector, 
                 d_obs: np.ndarray, 
                 contrl, 
                 states, 
                 device, 
        ...
        super().__init__(predictor, autoencoder, measurement, selector, 
                         d_obs, contrl, states, 
                         device, batch_size, regularization, 
                         update_scales, scale_index_map, z0)
...
class OptimizationStage:
    def __init__(self, 
        ...
        self.update_scales = config.get('update_scales')
        self.z0 = z0
    ...
    def select_latent_mask(self):
        ...
        density = self.measurement.density.detach().cpu().numpy()
    ...
    def build_proxy_optimizer(self):
        ...
        return GradientBasedProxyOptimizer(
            ...
            update_scales=self.update_scales,
            scale_index_map=self.scale_indices,
            z0=self.z0
        )
